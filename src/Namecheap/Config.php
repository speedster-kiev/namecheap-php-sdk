...
class Config
{
    public const ENV_SANDBOX = 'https://api.sandbox.namecheap.com/xml.response';
    public const ENV_PRODUCTION = 'https://api.namecheap.com/xml.response';
    ...
    protected function _checkKey($key)
    {
        $key = (string) $key;
        if (!array_key_exists($key, $this->_data)) {
            throw new NamecheapException(__CLASS__ . ' does not have the property ' . $key);
        }
    ...
    {
        $this->_data['apiKey'] = strtolower(substr($value, 0, 50));
    ...
    public function setClientIp($value)
    {
        if (!filter_var($value, FILTER_VALIDATE_IP)) {
            throw new NamecheapException('Invalid client ip ' . $value);
        }
    ...
    {
        if (!is_bool($value)) {
            throw new NamecheapException('Property sandbox must be bool (true/false)');
        }
